...
class BinomialDiffusion:
    ...
    def p_mean(
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_Y:
            ...
            model_mean = torch.where((t == 0)[:, None, None, None], pred_xstart, model_mean)
        else:
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        ...
        return (
            torch.abs(x_t - eps).to(device=t.device).float()
        )
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL or self.loss_type == LossType.MIX:
            ...
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            if self.model_mean_type == ModelMeanType.PREVIOUS_Y:
                target = self.q_posterior_mean(
                    x_start=x_start, x_t=x_t, t=t
                )
            elif self.model_mean_type == ModelMeanType.START_Y:
                target = x_start
            elif self.model_mean_type == ModelMeanType.EPSILON:
                target = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=x_start)
            else:
                raise NotImplementedError(self.model_mean_type)
